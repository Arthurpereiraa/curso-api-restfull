...
public class ProfessorController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> alterar(@PathVariable Long id, @RequestBody Professor p) {
		try {
			return ResponseEntity.status(200).body(professorService.alterar(id, p));
		} catch (Exception e) {
	...
	public ResponseEntity<?> gravar(@RequestBody Professor p) {
		try {
			return ResponseEntity.ok(professorService.gravar(p));
		} catch (Exception e) {
